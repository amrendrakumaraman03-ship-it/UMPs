...
export default function Reports() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Report Selector */}
      <div className="flex gap-2 bg-gray-100 p-1 rounded-xl" role="tablist" aria-label="Report Types">
          <button 
            role="tab"
            aria-selected={reportType === 'sales'}
            aria-controls="tabpanel-sales"
            id="tab-sales"
            tabIndex={reportType === 'sales' ? 0 : -1}
            onClick={() => setReportType('sales')}
            onKeyDown={(e) => {
              if (e.key === 'ArrowRight') { e.preventDefault(); document.getElementById('tab-gst')?.focus(); setReportType('gst'); }
              if (e.key === 'ArrowLeft') { e.preventDefault(); document.getElementById('tab-stock')?.focus(); setReportType('stock'); }
            }}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${reportType === 'sales' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
              Sales
          </button>
          <button 
            role="tab"
            aria-selected={reportType === 'gst'}
            aria-controls="tabpanel-gst"
            id="tab-gst"
            tabIndex={reportType === 'gst' ? 0 : -1}
            onClick={() => setReportType('gst')}
            onKeyDown={(e) => {
              if (e.key === 'ArrowRight') { e.preventDefault(); document.getElementById('tab-stock')?.focus(); setReportType('stock'); }
              if (e.key === 'ArrowLeft') { e.preventDefault(); document.getElementById('tab-sales')?.focus(); setReportType('sales'); }
            }}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${reportType === 'gst' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
              GST
          </button>
          <button 
            role="tab"
            aria-selected={reportType === 'stock'}
            aria-controls="tabpanel-stock"
            id="tab-stock"
            tabIndex={reportType === 'stock' ? 0 : -1}
            onClick={() => setReportType('stock')}
            onKeyDown={(e) => {
              if (e.key === 'ArrowRight') { e.preventDefault(); document.getElementById('tab-sales')?.focus(); setReportType('sales'); }
              if (e.key === 'ArrowLeft') { e.preventDefault(); document.getElementById('tab-gst')?.focus(); setReportType('gst'); }
            }}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${reportType === 'stock' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
              Stock Audit
          </button>
      </div>
